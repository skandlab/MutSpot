...
subtract.regions.from.roi.chr <- function(rois, sub,chr = 'chr10') {
  ...
  if (class(rois) == "GRangesList") {
    ...
    roi.chr = BiocGenerics::unlist(rois[as.character(GenomeInfoDb::seqnames(rois)) == chr]); sub.chr = BiocGenerics::unlist(sub[as.character(GenomeInfoDb::seqnames(sub)) == chr]);
    ...
    roi.chr = rois[GenomeInfoDb::seqnames(rois) == chr]; sub.chr = sub[GenomeInfoDb::seqnames(sub) == chr];
    GenomicRanges::elementMetadata(roi.chr) <- NULL; GenomicRanges::elementMetadata(sub.chr) <- NULL
    # return if one list is empty
    if (length(roi.chr) == 0 || length(sub.chr) == 0) {
