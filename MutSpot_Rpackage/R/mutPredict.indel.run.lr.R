...
mutPredict.indel.run.lr <- function(output.dir, merge.hotspots = TRUE, indel.mutations.file, fdr.cutoff = 0.05, color.line = "red", color.dots = "maroon1", color.muts = "orange", top.no = 3,
  ...
  process.single.roi <- function(x) {
    ...
    roi.feat.indel = mutPredict.indel.get.features(GenomicRanges::reduce(mut.regions[x.idx]), continuous.selected.features.indel, discrete.selected.features.indel, genome.build = genome.build)
